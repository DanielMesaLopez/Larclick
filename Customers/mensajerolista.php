...
<?php
 ...
 include("config.php");
  ...
  if(isset($_GET['delete_id']))
  {
    ...
    header("Location: mensajerolista.php");
  }
...
<?php
  ...
  if(isset($_GET['update_id']))
  {
    ...
    $stmt_delete = $DB_con->prepare('update detallepedido set estado_pedido="Ordered" WHERE estado_pedido="Pending" and id_cliente =:id_cliente');
    $stmt_delete->bindParam(':id_cliente',$_GET['update_id']);
    $stmt_delete->execute();
    echo "<script>alert('Productos Pedidos Exitosamente!')</script>";

    header("Location: mensajeropedido.php");
  }
...
<main class="container">
 ...
 <div id="page-wrapper">
      ...
      <div class="alert alert-default" style="color:white;background-color:#C70039">
         <center><h3> <span class="fa fa-cart-plus"></span> Lista de Compras</h3></center>
        </div>
      ...
      <br />
            ...
            <table class="display table table-bordered" id="example" cellspacing="0" width="100%" style="color:black;background-color:#ffffff">
              <thead>
                <tr>
                  ...
                  <th><center><h4>Establecimiento</h4></center></th>
          ...
          <th><center><h4>Total</h4></center></th>
                  <th><center><h4>Acción</h4></center></th>
...
include("config.php");
  ...
  $stmt = $DB_con->prepare("SELECT * FROM detallepedido where estado_pedido='Pending' and id_cliente='$id_cliente'");
  $stmt->execute();
    ...
    while($row=$stmt->fetch(PDO::FETCH_ASSOC))
    {
      ...
      ?>
         ...
         <td>
                  ...
                  <a class="btn btn-block btn-danger" href="?delete_id=<?php echo $row['id_pedido']; ?>" title="click for delete" onclick="return confirm('¿Estás segura de eliminar este producto?')"><h4><span class='fas fa-trash-alt'></span> Eliminar</h4></a>
     ...
     include("config.php");
      $stmt_edit = $DB_con->prepare("select sum(total_pedido) as totalx from detallepedido where id_cliente=:id_cliente and estado_pedido='Pending'");
    $stmt_edit->execute(array(':id_cliente'=>$id_cliente));
    $edit_row = $stmt_edit->fetch(PDO::FETCH_ASSOC);
    extract($edit_row);

    echo "<tr>";
    echo "<td colspan='3' align='right'><h4>Total Precio del Domicilio: ";
    echo "</td></h4>";

    echo "<td><h4>&dollar; ".$totalx;
    echo "</td></h4>";

    echo "<td>";
    echo "<a class='btn btn-block btn-success' href='?update_id=".$id_cliente."' ><h4><span class='fa fa-shopping-cart'></span> Realizar Pedido!</h4></a>";
    echo "</td>";

    echo "<td>";
    echo "<a class='btn btn-block btn-danger' href='mensajero.php?id=1' ><h4><span class='fa fa-shopping-cart'></span> Seguir Comprando!</h4></a>";
    echo "</td>";
    ...
    ?>
        ...
        <div class="col-xs-12">
          <div class="alert alert-warning">
             <h4> <span class="fas fa-info-circle"></span> &nbsp; No se ha encontrado ningún Producto ...</h4>
            </div>
        </div>
        <?php
  }
